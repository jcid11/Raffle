...
class playStream extends StatelessWidget {
  ...
  Widget build(BuildContext context) {
    return StreamBuilder<QuerySnapshot>(
      stream: _firestore.collection('Entrance').where('user',isEqualTo: _auth.currentUser!.email).orderBy('date').snapshots(),
        ...
        if(playsMadeLength>0){
          return ListView.builder(
            ...
            itemBuilder: (BuildContext context, int index) {
              ...
              return ListTile(title: tileContainerStyle(tile: Text('${doc.get('number')} => ${doc.get('itemName')} => ${doc.get('date')} => ${doc.get('itemPrice')}',style: TextStyle(color: Colors.white),)),);
            },
          );
        }else{
          return Padding(
...
class tileContainerStyle extends StatelessWidget {
  final Text tile;

  const tileContainerStyle({Key? key, required this.tile}) : super(key: key);
